...
import { ProviderClaimsResponse } from "@/types";
import { ProviderClaim } from "@/types";

interface ProviderClaimsMetricsProps {
  claimsData?: ProviderClaimsResponse;
  isLoading?: boolean;
}
...
const SkeletonProviderClaimsMetrics = () => {
  return (
...
export const ProviderClaimsMetrics = ({
  claimsData,
  isLoading,
}: ProviderClaimsMetricsProps) => {
  if (isLoading) {
    return <SkeletonProviderClaimsMetrics />;
  }
  ...
  const calculateMetrics = () => {
    ...
    claims.forEach((claim: ProviderClaim) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
      <ValueIndicator
        ...
        description="All claims for this provider."
      />
